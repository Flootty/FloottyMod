package floottymod.floottymod.util.search;
...
public final class ChunkSearcher {
    ...
    public ChunkSearcher(Chunk chunk, List<Block> blocks, int dimensionId) {
        this.chunk = chunk;
        this.blocks = blocks;
    ...
    public void startSearching(ExecutorService pool) {
        if(status != Status.IDLE) throw new IllegalStateException();
    ...
    private void searchNow() {
        if(status == Status.IDLE || status == Status.DONE || !matchingBlocks.isEmpty()) throw new IllegalStateException();

        ChunkPos chunkPos = chunk.getPos();
        ClientWorld world = FloottyMod.MC.world;
        ...
        for(int x = minX; x <= maxX; x++)
            for(int y = minY; y <= maxY; y++)
                for(int z = minZ; z <= maxZ; z++) {
                    if(status == Status.INTERRUPTED || Thread.interrupted()) return;

                    BlockPos pos = new BlockPos(x, y, z);
                    Block block = BlockUtils.getBlock(pos);

                    for(Block b : blocks) if(b.equals(block)) matchingBlocks.add(pos);;
                }
        status = Status.DONE;
    }
