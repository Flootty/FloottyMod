...
import floottymod.floottymod.mixininterface.IClientPlayerEntity;
...
public enum RotationUtils {
	...
	public static Vec3d getServerLookVec() {
		RotationFaker rotationFaker = FloottyMod.INSTANCE.getRotationFaker();
	...
	public static double getAngleToLastReportedLookVec(Vec3d vec) {
		...
		IClientPlayerEntity player = FloottyMod.IMC.getPlayer();
	...
	public static float getHorizontalAngleToLookVec(Vec3d vec) {
		Rotation needed = getNeededRotations(vec);
		return MathHelper.wrapDegrees(FloottyMod.MC.player.getYaw())
			- needed.yaw;
	}
